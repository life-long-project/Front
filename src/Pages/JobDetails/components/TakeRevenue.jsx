...
export default function TakeRevenue({ isOpen, onOpen, onClose, job }) {
  ...
  const handleSendRating = async () => {
    try {
      const res = await axios.post(
        `https://back-ph2h.onrender.com/rate/job/${job}/?auth_token=${localStorage.getItem(
          "token"
        )}`,
        userRating
      );
  ...
  return (
    <>
      <Modal closeOnOverlayClick={false} isOpen={isOpen} onClose={onClose}>
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>Take Revenue!</ModalHeader>
          <ModalCloseButton />
          <ModalBody pb={6}>
          ...
          <ModalFooter>
            <Button onClick={() => {handleSendRating()}} colorScheme="green" mr={3}>
              Get Revenure
            </Button>
